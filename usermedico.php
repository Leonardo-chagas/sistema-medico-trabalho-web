...
<html>
    ...
    <body id="background">
        ...
        <ul id="navbar">
            <li class="nav"><a id="active" href="usermedico.php">Consultas</a></li>
        ...
        <div id="dados">
            ...
            <?php
                ...
                $sql = "SELECT COUNT(*) FROM consultas WHERE medico LIKE '$nome'";
                $result = $conn->query($sql);
                $exists = ($result->fetchColumn() > 0) ? true : false;

                if($exists){
                    echo '<table style="position: relative; top: 0; bottom: 0; left: 0; right: 0; width: 100%;" id="contents">
                    <tr style="font-weight: bold;"> <th>Data</th> <th>Paciente</th> <th>Receitas</th> <th>Observações</th> </tr>';
                    $sql = "SELECT * FROM consultas WHERE medico LIKE '$nome'";
                    $result = $conn->query($sql);
                    $data = $result->fetchAll();
                    foreach($data as $row){
                        echo "<tr> <th>".$row['dia']."</th> <th>".$row['paciente']."</th> <th>".$row['receita']."</th> <th>".$row['observações']."</th> </tr>";
                    }
                    echo "</table>";
                }
                else{
                    echo '<p>Não existe nenhuma consulta cadastrada para esta conta</p>';
                }
                $conn = null;
            ?>
        ...
        <span>Total de consultas mensais: <?php
            session_start();
            $server = 'localhost';
            $user = 'root';
            $password = '';
        ...
        <span>Total de consultas anuais: <?php
            session_start();
            $server = 'localhost';
            $user = 'root';
            $password = '';
