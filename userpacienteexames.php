...
<html>
    ...
    <body id="background">
        ...
        <ul id="navbar">
            <li class="nav"><a id="active" href="userpacienteexames.php">Exames</a></li>
        ...
        <div id="dados">
            ...
            <?php
                ...
                $sql = "SELECT COUNT(*) FROM exames WHERE paciente LIKE '$nome'";
                $result = $conn->query($sql);
                $exists = ($result->fetchColumn() > 0) ? true : false;

                if($exists){
                    echo '<table style="position: relative; top: 0; bottom: 0; left: 0; right: 0; width: 100%;" id="contents">
                    <tr style="font-weight: bold;"> <th>Data</th> <th>Laboratório</th> <th>Tipo de Exame</th> <th>Resultado</th> </tr>';
                    $sql = "SELECT * FROM exames WHERE paciente LIKE '$nome'";
                    $result = $conn->query($sql);
                    $data = $result->fetchAll();
                    foreach($data as $row){
                        echo "<tr> <th>".$row['dia']."</th> <th>".$row['laboratorio']."</th> <th>".$row['tipo']."</th> <th>".$row['resultado']."</th> </tr>";
                    }
                    echo "</table>";
                }
                else{
