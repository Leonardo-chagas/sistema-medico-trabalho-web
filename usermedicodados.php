...
<html>
    ...
    <body id="background">
        ...
        <ul id="navbar">
            ...
            <li class="nav"><a href="usermedicocadastro.php">Cadastrar Consulta</a></li>
            <li class="nav"><a id="active" href="usermedicodados.php">Usuário</a></li>
        </ul>
        <br><br>

        <div class="formdiv">
            <br>
        <form method="post" action="changemedico.php" id="insertData">
            <?php
            session_start();
            $server = 'localhost';
            $user = 'root';
            ...
            $sql = "SELECT * FROM medicos WHERE nome LIKE '$nome'";
            $result = $conn->query($sql);
            $data = $result->fetch();

            echo '<span>Nome: </span><input type="text" placeholder="Nome" name="mNome" id="mNome" value='.$data['nome'].' class="addinput">
            ...
            <span>Especialidade: </span><input type="text" placeholder="Especialidade" name="mEspecialidade" id="mEspecialidade" value='.$data['especialidade'].' class="addinput">
                ...
                if(proceed){
                    ...
                    .done(function(response){
                        if(response.type == 'nameError'){
                            $("#nomeError").css('display', 'inline');
                            $("#nomeError").html('Já existe um médico com este nome');
                        }
                        else if(response.type == "telefoneError"){
                            $("#telefoneError").css('display', 'inline');
                            $("#telefoneError").html('Já existe um médico com este telefone');
                        }
