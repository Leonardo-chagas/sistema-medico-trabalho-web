...
<html>
    ...
    <body id="background">
        ...
        <ul id="navbar">
            ...
            <li class="nav"><a id="active" href="userpacienteconsultas.php">Consultas</a></li>
            <li class="nav"><a href="userpacientedados.php">Usuário</a></li>
        </ul>
        <br><br>
        ...
        <div id="dados">
            ...
            <?php
                ...
                $sql = "SELECT COUNT(*) FROM consultas WHERE paciente LIKE '$nome'";
                $result = $conn->query($sql);
                $exists = ($result->fetchColumn() > 0) ? true : false;

                if($exists){
                    echo '<table style="position: relative; top: 0; bottom: 0; left: 0; right: 0; width: 100%;" id="contents">
                    <tr style="font-weight: bold;"> <th>Data</th> <th>Médico</th> <th>Receitas</th> </tr>';
                    $sql = "SELECT * FROM consultas WHERE paciente LIKE '$nome'";
                    $result = $conn->query($sql);
                    $data = $result->fetchAll();
                    foreach($data as $row){
                        echo "<tr> <th>".$row['dia']."</th> <th>".$row['medico']."</th> <th>".$row['receita']."</th> </tr>";
                    }
                    echo "</table>";
                }
                else{
